...
class AorUI:
    ...
    def task(self, version, path):
        ...
        if os.path.exists(jarPath) and os.path.exists(jsonPath) and os.path.exists(assetIndexPath):
            self.step1="Step 1: 解压{}.jar\n".format(version)
            self.content.configure(text=self.step1)
            with zipfile.ZipFile(os.path.join(self.minecraftdir.get(), "versions", version, version + ".jar")) as jar:
                ...
                self.jar_progress = 0
                files_len = len(files)
                self.content.configure(text=self.step1+"0% ({}/{})".format(self.jar_progress, files_len))
                cpu_count = os.cpu_count()

                # 计算每个线程应该处理的文件数量
                elements_per_thread = files_len // cpu_count
                # 计算剩余的文件数量
                remaining_elements = files_len % cpu_count
                ...
                for i in range(cpu_count):
                    ...
                    # 获取当前线程应该处理的子列表
                    sublist = files[start_index:end_index]
                    # 创建并启动线程
                    thread = Thread(target=self.extract_files, args=(sublist, path, version, files_len))
                    thread.start()
    ...
    def extract_files(self, files, path, version, files_len):
        with zipfile.ZipFile(os.path.join(self.minecraftdir.get(), "versions", version, version + ".jar")) as jar:
            for file in files:
                jar.extract(file, path)
                self.jar_progress += 1
                self.prog.set((self.jar_progress/files_len)*1000)
                self.content.configure(text=self.step1+"{}% ({}/{})".format(round((self.jar_progress/files_len)*100), self.jar_progress, files_len))
